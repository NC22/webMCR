...
function DeleteComment(id) {
    ...
    var event = function(response) {
        ...
        var trash = GetById('comment-byid-' + id)
...
function PostComment(script) {
    ...
    if (text == null || item_id == null || item_type == null || item_id.value <= 0)
        return false
    if (text.length < 1)
        return false
    ...
    var event = function(response) {
        ...
        if (response['code'] == 0) {
            ...
            if (response['comment_revers'])
                ...
                insertInBegin(new_comment, GetById('comments-main'))
            
            fadeElement(new_comment, function() {}, 'in');
            setBootstrapEvents();
        }
        ...
        if (response['code'] != 0) {
            ...
            BlockVisible('comment-error', false)
...
function Register() {
    ...
    if (login == null || pass == null || pass2 == null || email == null || female == null)
        return false
    if (pass.length < 1 || pass2.length < 1 || login.length < 1 || email.length < 1)
        return false
...
function Restore() {
    ...
    if (email.length < 1 || code.length != 4)
        return false
    ...
    var event = function(response) {
        ...
        if (response['code'] == 0)
            messageBoxText.className = 'alert alert-success';
...
function DeleteFile(id) {
    ...
    var event = function(response) {
        ...
        var fileTrash = GetById('file-' + id)
        fadeElement(fileTrash, function() {
            if (fileTrash != null)
                fileTrash.parentNode.removeChild(fileTrash)
        }, 'out');
    }
...
function UpdateProfile(admTrg) {
    ...
    var event = function(response) {
        ...
        Ava.onload = function() {
            ...
            if (skin_front != null)
                skin_front.style.backgroundImage = 'url(' + ava_link + ')'
            var skin_back = GetById('profile-skin-back')
            if (skin_back != null)
                skin_back.style.backgroundImage = 'url(' + ava_link + ')'
