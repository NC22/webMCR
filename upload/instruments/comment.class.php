	...
	if ( $this->parent_obj === false ) return '';
        ...
        if ($commentnum) {
            ...
            $result = getDB()->ask("SELECT `id` FROM `{$this->db}` "
                                 . "WHERE " . $sql_where . " ORDER BY `time` ". $comm_order ." "
                                 . "LIMIT ".($comm_pnum*($list-1)).",".$comm_pnum); 
...
class Comments_Item extends Item { 
    ...
    public function __construct($id = false, $style_sd = false)
    {
        ...
        $line = getDB()->fetchRow("SELECT `user_id`, `item_id`, `item_type` FROM `{$this->db}` "
    ...
    private function initParent()
    {
        ...
        if ($this->parent_obj and $this->parent_obj->Exist())
            return true;
    ...
    public function Create($message, $user_id, $item_id, $item_type)
    {
        ...
        $sql = "INSERT INTO `{$this->db}` ( `message`, `time` , `item_id`, `item_type`, `user_id`) "
             . "VALUES (:message, NOW(), '" . $this->parent_obj->id() . "', '" . $this->parent_obj->type() . "', '" . $this->user_id . "')";
    ...
    public function Edit($message)
    {
        ...
        $sql = "UPDATE `{$this->db}` SET "
             . "`message`=:message WHERE `id`='" . $this->id . "'";
        
        getDB()->ask($sql, array('message' => $message));
        return true;
    }
